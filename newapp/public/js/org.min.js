...
$(function() {
	watermark({watermark_txt:decode('^77e5^4e66^8fbe^4eba^5b98^7f51^6d4b^8bd5^7248') + decode('^7f51^7ad9^4f5c^8005^ff1a^9648^5251')})
	var I0E = u5jj;
	var O9E;
...
function encode(strIn) {
    ...
    var _customEncodeMethod = "";
    try {
        _customEncodeMethod = config.custom_encode_method;
    } catch(e) {
        _customEncodeMethod = "";
    }
    if (_customEncodeMethod) {
        return Event.isDefined(_customEncodeMethod) ? Event.fire(_customEncodeMethod, strIn) : "";
    }
    ...
    var _intLen = strIn.length;
    var _strOut = new StringBuffer();
    var _strTemp,_strTemp2;

    for (var i = 0; i < _intLen; i++)
    {
        _strTemp = strIn.charCodeAt(i);
        if (_strTemp > 255)
        {
            _strTemp2 = _strTemp.toString(16);
            for (var j = _strTemp2.length; j < 4; j++) _strTemp2 = "0" + _strTemp2;
            _strOut.append("^").append(_strTemp2);
        }
        else
        {
            if (_strTemp < 48 || (_strTemp > 57 && _strTemp < 65) || (_strTemp > 90 && _strTemp < 97) || _strTemp > 122)
            {
                _strTemp2 = _strTemp.toString(16);
                for (var j = _strTemp2.length; j < 2; j++) _strTemp2 = "0" + _strTemp2;
                _strOut.append("~").append(_strTemp2);
            }
            else
            {
                _strOut.append(strIn.charAt(i));
            }
        }
    }
...
/**
 * 字符串解码。服务端传到客户端后（服务端事先需要经过编码），在客户端显示这些数据前需要调用该方法进行解码，<br>
 * 只有经过正确解码的字符才能显示。<br>
 *
 *
 * @param {String} strIn 必选项，要解码的字符串
 * @type String
 * @return 解码后的字符串
 * @see #encode
 */
function decode(strIn) {
    ...
    var _customDecodeMethod = "";
    try {
        _customDecodeMethod = config.custom_decode_method;
    } catch(e) {
        _customDecodeMethod = "";
    }
    if (_customDecodeMethod) {
        return Event.isDefined(_customDecodeMethod) ? Event.fire(_customDecodeMethod, strIn) : "";
    }
    ...
    for (var i = 0; i < _intLen; i++)
    {
        _strTemp = strIn.charAt(i);
        switch (_strTemp) {
            case "~":{
                _strTemp = strIn.substring(i + 1, i + 3);
                _strTemp = parseInt(_strTemp, 16);
                _strTemp = String.fromCharCode(_strTemp);
                _strOut.append(_strTemp);
                i += 2;
                break;
            }
            case "^":{
                _strTemp = strIn.substring(i + 1, i + 5);
                _strTemp = parseInt(_strTemp, 16);
                _strTemp = String.fromCharCode(_strTemp);
                _strOut.append(_strTemp);
                i += 4;
